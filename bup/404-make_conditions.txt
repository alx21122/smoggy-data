...
def _bootstrap_guess():
    """MONO gia to APOLYTOS PROTO run pote (den yparxei akoma
    .last_auto_run) -- xoris proigoumeno simeio anaforas, den exoume apo pou
    na metrisoume +6h. Mia-mia-fora ektimisi apo to roloi (nearest past
    6-hourly slot, UTC). An einai ligo lathos (px to ECMWF den exei akoma
    dimosiefsei to slot pou mantepsame), den peirazei -- NotReadyYet, kai to
    epomeno polling tick (30 lepta meta) ksanadokimazei to IDIO slot mexri
    na vgei, xoris kanena allo provlima. Meta apo AYTO to PROTO successful
    run, auti i synartisi den xanaxrisimopoieitai POTE -- to +6h anelamvani
    apo edo kai pera."""
    now = dt.datetime.now(dt.timezone.utc)
    h = (now.hour // 6) * 6
    return dt.datetime(now.year, now.month, now.day, h, 0,
