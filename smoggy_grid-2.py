...
# ---------------------------------------------------------------- POLLA IPSI (#053 fix)
# GR: IDIA logiki me to _rhiAt() sto analyze.js (server-side, "AUTO" selector):
#     gia KATHE pixel, anamesa sta SAC-valid ipsi (T < Tcrit tou ipsomatou), diali
#     to ipsos me to MEGALYTERO RHi. An KANENA ipsos den einai SAC-valid -> state 0.
#     Xrisimopoiei AYTOUSIA ta rhi()/t_crit()/ct_state() apo pano — kanena neo
#     typos fysikis, mono composition/epilogi.
# GUARD: an allaksei to rhi()/t_crit()/ct_state(), allazei kai edo automata (idio
#     kalesma) — DEN yparxei diplotypos typos edo pou na xreiazetai xechoristi syntirisi.
def ct_state_multilevel(t_c_list, rh_list, levels_hpa):
    """
    t_c_list, rh_list: listes apo 2D arrays (idio sxima to kathena), MIA ana
    ipsos, idia seira me to levels_hpa (π.χ. [400,300,250,200]).
    Epistrefei 2D uint8 state (idio sxima) — to state tou NIKITI ipsomatou
    ana pixel (max RHi anamesa sta SAC-valid).
    """
    assert len(t_c_list) == len(rh_list) == len(levels_hpa), \
        "t_c_list/rh_list/levels_hpa prepei na exoun idio mikos"
    shape = np.asarray(t_c_list[0]).shape
    best_rhi = np.full(shape, -np.inf, dtype=np.float64)
    best_state = np.zeros(shape, dtype=np.uint8)
    any_valid = np.zeros(shape, dtype=bool)

    for t_c, rh, hpa in zip(t_c_list, rh_list, levels_hpa):
        t_c = np.asarray(t_c, dtype=np.float64)
        rh = np.asarray(rh, dtype=np.float64)
        tc = t_crit(hpa * 100.0)
        if tc is None:
            continue                      # afto to ipsos POTE den ikanopoiei SAC (fysika)
        sac = t_c < tc
        ri = rhi(rh, t_c)
        st = ct_state(t_c, hpa, ri)       # to state AN mono autos o ipsos ithele

        better = sac & (ri > best_rhi)    # kalytero RHi ANAMESA sta SAC-valid mono
        best_rhi = np.where(better, ri, best_rhi)
        best_state = np.where(better, st, best_state).astype(np.uint8)
        any_valid = any_valid | sac

    return np.where(any_valid, best_state, 0).astype(np.uint8)
...
BLUE = (0x44, 0xAA, 0xFF)
ALPHA = {0: 0, 1: 0, 2: 105, 3: 165}   # 1 = vraxyvio -> DEN vafetai (apofasi Alex 26/7)
